    ...
    const adminPanel = isEditing ? `
      <div class="admin-panel" data-admin-id="${safeId}">
        <div class="admin-grid">
          <label>店名<input class="admin-input" data-admin-field="name" value="${escapeHtml(item.name || '')}"></label>
          <label>分類<input class="admin-input" data-admin-field="category" value="${escapeHtml(item.category || '')}"></label>
          <label>地區<input class="admin-input" data-admin-field="area" value="${escapeHtml(item.area || '')}"></label>
          <label>價位<input class="admin-input" data-admin-field="price" value="${escapeHtml(item.price || '')}"></label>
          <label>評分
          ...
          <label>IG 影片<input class="admin-input" data-admin-field="ig" value="${escapeHtml(item.ig || '')}"></label>
          <label>YouTube 影片<input class="admin-input" data-admin-field="youtube" value="${escapeHtml(item.youtube || '')}"></label>
          <label class="admin-field">IG 留言
  ...
  if (isAdmin){
    ...
    cardsEl.querySelectorAll('[data-admin-delete]').forEach(btn=>{
      btn.onclick = async ()=>{
        ...
        const id = wrap.getAttribute('data-admin-id');
        if (!id) return;
        if (!confirm('確定要刪除這筆餐廳嗎？刪除後無法復原。')) return;
        const msgEl = wrap.querySelector('[data-admin-msg]');
        const setMsg = (text)=>{
          if (!msgEl) return;
          msgEl.textContent = text || '';
